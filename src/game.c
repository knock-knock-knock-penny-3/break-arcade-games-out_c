...
internal void spawn_particle_explosion(int count, v2 p, f32 dp_scale, f32 base_size, f32 base_life, u32 color) {
...
void simulate_game(Game *game, Input *input, f64 dt) {
    ...
    for_each_ball {
        ...
        if (ball->dp.y < 0 && is_colliding(player_visual_p, player_half_size, ball->desired_p, ball->half_size)) {
            ...
            spawn_particle_explosion(10, ball->desired_p, 8.f, 1.f, .15f, ball->color);
            touchless_bonus = 0;
...
//            f32 speed_t = map_into_range_normalized(2500, len_sq(ball->dp), 100000);
            f32 speed_t = map_into_range_normalized(2500, len_sq(ball->dp), 250000);
            ball->trail_spawner_t += 50.f / len_sq(ball->dp);

            RGBA color = {
                map_color_into_range_normalized(0x00, speed_t, 0x33),
                map_color_into_range_normalized(0xBB, speed_t, 0xFF),
                map_color_into_range_normalized(0xEE, speed_t, 0xFF),
                0xFF
            };
            f32 life = .32f;
            f32 dp = 2.f;
